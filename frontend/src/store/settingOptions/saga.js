...
import {
  SettingOptionsApiResponseSuccess,
  SettingOptionsApiResponseError,
...
function* getSettingOptions() {
  ...
  try {
    ...
    yield put(SettingOptionsApiResponseSuccess(GET_SETTING_OPTIONS, response.data));
  } catch (error) {
    yield put(SettingOptionsApiResponseError(GET_SETTING_OPTIONS, error));
  }
}
