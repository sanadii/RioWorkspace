 ...
 * @param {*} param0
 */
function* changeBackgroundImageType({ payload: backgroundImageType }) {
    try {
        yield call(changeHTMLAttribute, "data-background-image-type", backgroundImageType);
    } catch (error) {
        // console.log(error);
    }
}
...
export function* watchChangeBackgroundImageType() {
    yield takeEvery(CHANGE_BACKGROUND_IMAGE_TYPE, changeBackgroundImageType);
}
...
function* LayoutSaga() {
    yield all([
        ...
        fork(watchChangeBackgroundImageType),
